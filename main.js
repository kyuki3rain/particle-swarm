...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  // Use unbiased normalization: when vel≈0 velNorm=(0,0), so syncForce=fieldVec*scale
  // (avoids the (1/√2, 1/√2) bias of vec2<f32>(0.0001) splat that caused leftward drift)
  let velLen = length(p.vel);
  let velNorm = select(vec2<f32>(0.0), p.vel / velLen, velLen > 1e-5);
  ...
  // --- integrate ---
  // Edge treatment: toroidal wrap-around (no wall force needed).
  // This is the standard approach for open swarm sims — particles seamlessly
  // reappear on the opposite side, giving an infinite-space feel.
